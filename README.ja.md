...
**ステータス**: 安定運用中(v1.1)。主要機能は実装完了し、現在は一区切り。以降はメンテナンス(データソース保守・バグ修正)が中心です。
...
- **疾患フィルタ** — エボラ、麻疹、デング熱、ハンタウイルス、エムポックス、サイクロスポラ症(v1.1 で追加)など16疾患。鳥インフルエンザを亜型別(H5N1 / H7N9 / H9N2 / 亜型不明)に分けているため、選択肢は19件になります
- **配信元ホワイトリスト** — 公共放送・全国紙・通信社・公的機関のみ
- **国名抽出** — 表記揺れ辞書(80カ国以上)による国名・地名 → ISO3 変換。複数国対応、広域・不明セクションあり。国名照合の後段で、以下のフォールバックが順に動作します:
  - 日本の地名(47都道府県・約1,285市区町村) → JPN
  - 日本の組織名(省庁・大学・制度用語) → JPN
  - 米国州名 → USA(誤検出ガード付き。曖昧な州名は「州」等の明示が必要)
  - 略記国名の判定(「米国」「米で」等の限定パターンのみ)
  - 広域マーカー(Multi-country / Multi-locations / Global / アフリカ 等) → `scope="broad"`
  - 疾患別の既定国による推定(暫定設定。現在はサイクロスポラ症 → USA のみ)。推定による判定は記事一覧の表に「(推定)」と表示し、確定判定と区別できます。詳細は「国の推定(暫定設定)」節を参照
...
- **疾患別の既定国は流行状況に依存する暫定設定**です。流行地が変われば誤った推定になるため、
  定期的な見直しが必要です。設定は `src/data/disease_defaults.py` の `DISEASE_DEFAULT_COUNTRY`
  1 か所にまとまっており、ここのエントリ(キー: 日本語の疾患名 / 値: ISO3 コード)を
  編集・削除するだけで変更できます。詳細は「国の推定(暫定設定)」節を参照。
- タイトルに国名・地名・広域マーカーのいずれも含まない記事は「地域不明」に分類されます。
- Yahoo ニュースからの取得が現在0件です(要調査)。同じ話題は他ソース経由で取得できているため、
  運用上の支障はありません。
- 翻訳は argos-translate によるローカル翻訳のため、訳文が不自然になる場合があります。
- インストールは Apple Silicon では `uv sync --extra translation`、Intel Mac では `uv sync`
  (翻訳機能なし)を使用してください。詳細は[動作環境](#動作環境)を参照。
...
### 2026-08-05 (4回目) — v1.1
- v1.1 として一区切り。ステータス行と機能一覧を更新
- 本リリースの内容(詳細は以下の3エントリを参照):
  - サイクロスポラ症を対象疾患に追加(16疾患に拡張)。※2026年夏、米国でアイスバーグレタスを介した大規模アウトブレイクが発生中
  - 米国州名 → USA、日本の地名・組織名 → JPN のマッピングを追加
  - 略記国名(「米で」「米国」等)の判定に対応
  - 記事タイトルへの写真クレジット混入(/Getty Images/File 等)を除去
  - 疾患別の既定国による国推定を導入。推定判定は表に「(推定)」と明示
  - 推定成功時の不要な WARNING を抑止し、推定ログのロガーを統一
- 「既知の制約」セクションを新設
